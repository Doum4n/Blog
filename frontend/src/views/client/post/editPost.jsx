...
const EditPost = () => {
    ...
    const UploadPost = async () => {
        await fetch('http://0.0.0.0/post/update', {
            method: 'PUT',
            headers: {
                'Content-Type' : 'application/json'
            },
        ...
        const fetchData = async () => {
            try{
                const [forumsRes, postRes] = await Promise.all([
                    ...
                    fetch(`http://0.0.0.0/get-post/${id}`).then((response) => response.json())
                ])

                setForums(forumsRes);
                setPost(postRes.post);

                setHtmlContent(postRes.post.content);
            }catch(err) {
    ...
    return (
        <Container className='mt-3'>
            <Form >
                <h2></h2>
                <Form.Control defaultValue={post.title} type='text' placeholder='Title' className='mb-3' onChange={(e) => setTitle(e.target.value)}/>
                ...
                <box style={style}>
                    <Form ref={FormRef}>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control defaultValue={post.tag_name} name="TagName" placeholder="hashtag"/>
                        </Form.Group>
                        <Form.Group className="mb-3">
